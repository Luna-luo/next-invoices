...
export async function createInvoice(prevState: State, formData: FormData){
  ...
  // If validation fails, return an error message early. Otherwise, continue.
  ...
  // prepare form data insert into database
  ...
  // revalidate the cache for the invoices page and redirect to the user.
...
export async function updateInvoice(id: string, prevState: State, formData: FormData){
  ...
  // prepare form data to update the database
  const {customerId, amount, status} = validatedFileds.data;
  const amountInCents = amount * 100;
